...
| `multi-user agent data` | `workspace_kind`, `agent_workspaces`, `get_cron_jobs`, `get_memories`, `get_issues`, `find_api_key`, `_openclaw_*` / `_hermes_*` | read each `DASHBOARD_USERS` user's workspace — `~/.openclaw` or `~/.hermes` (Hermes Agent), picked per user by `workspace_kind`; `agent_workspaces` returns which users have either |
| `model / profile config` | `_fmt_model`, `_parse_profile`, `_load_profiles`, `_read_yaml`, `_hermes_profiles`, `get_model_config` | per-agent model/profile tables from `agents/<agent>/[agent/]{MODEL_CONFIG_FILES}` (openclaw, JSON) or `config.yaml` + `profiles/<name>/config.yaml` (Hermes, YAML); deliberately tolerant parser |
| `API test` | `test_api` | one-shot latency probe to Anthropic / Google / Moonshot via raw `urllib` |
| `speed test` | `speedtest_flavor`, `find_speedtest`, `do_speedtest` | speed-test CLI wrapper (~20 s, blocking), `SPEEDTEST_COOLDOWN` between runs (default 3 min); auto-detects the Ookla CLI or the Python `speedtest-cli` |
...
- **Two workspace layouts, one per user:** `workspace_kind(user)` returns `'openclaw'` (`~/.openclaw`), `'hermes'` (`~/.hermes`, [Hermes Agent](https://github.com/NousResearch/hermes-agent)), or `None`; every reader dispatches on it and never merges the two. Each reader's openclaw branch stayed byte-identical when Hermes was added — keep it that way, and note the `None` case must keep falling through to the openclaw branch so a user with no workspace behaves exactly as before. Hermes specifics: cron timestamps are ISO strings (`_local_stamp`) not epoch ms, memories are two standing files not one per day, there is no ISSUES.md equivalent, and model config is YAML translated onto the openclaw profile keys so `_parse_profile` formats both.
- **PyYAML is optional at runtime:** `import yaml` is guarded and `_read_yaml` returns `None` when it failed, so a host that updated `dashboard.py` without re-running pip loses the Hermes model table only. Don't turn it into a hard import.
