...
- **Accept:** the panel works with either CLI installed under any of those names; with none installed the panel and the startup log both say what to install; the cooldown is not burned by a failed run.
...
### D1.3 Speed-test cooldown 1 h → 3 min
- **Where:** `SPEEDTEST_COOLDOWN`, `claim_speedtest`, the speed-test card title.
